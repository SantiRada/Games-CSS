<?php
    ...
    $msg = $_GET['msg'] ?? "";
    $level = $_GET['level'] ?? 1;
    $row = mysqli_fetch_assoc(mysqli_query($con, "select * from leveldata where idgame = '4' and numlevel = '".$level."';"));

    $resall = mysqli_query($con, "select * from leveldata where idgame = '4' order by numlevel asc;");
    ...
    if(isset($_POST['next-level'])):
        $verify = mysqli_query($con, "select * from leveldata where idgame = '4' and numlevel = '".($level + 1)."';");
    ...
    <form method="post">
        <main class="distribution">
            <section id="left-content">
                ...
                <div class="codes">
                        <textarea id="input-1" class="input" placeholder="Direccionamiento..." rows="5"></textarea>
                </div>

                <footer class="footer">
                    <p>Pathfinder es una creación de <span>Santiago Rada</span> para las clases de la <strong>UTN</strong></p>
                </footer>
            </section>
